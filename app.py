...
def average_travel_time(routeset):      # Reducir tiempo de viaje promedio de cada pasajero
    total_time = 0
    for route in routeset:
        for i in range(len(route) - 1):
            total_time += D[route[i]][route[i + 1]]
    return total_time / len(routeset.size)


def total_travel_time_savings(routeset):    # Calcular el tiempo de viaje total que se ahorra en cada demanda
    total_savings = 0
    for route in routeset:
        for i in range(len(route) - 1):
            total_savings += D[route[i]][route[i + 1]]
